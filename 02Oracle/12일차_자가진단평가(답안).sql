...
INSERT INTO employees (empId, name_, ssn, hiredate, phone, region, department, job_,  basicpay, extrapay)
    SELECT num, name, ssn, ibsaDate, tel, city, buseo, jikwi, basicPay, sudang
    FROM insa;
...
INSERT INTO regions (regId, reg_name)
    SELECT CONCAT('reg',LPAD(ROWNUM,3,'0')) regid, city 
    FROM (SELECT DISTINCT city FROM insa);
...
INSERT INTO departments (deptId, dept_name)
    SELECT CONCAT('dept',LPAD(ROWNUM,3,'0')) deptId, buseo
    FROM (SELECT DISTINCT buseo FROM insa);
...
INSERT INTO jobs (jobId, job_title)
    SELECT CONCAT('job',LPAD(ROWNUM,3,'0')) jobId, jikwi 
    FROM (SELECT DISTINCT jikwi  FROM insa);
...
-----------------------------------
/*
문제9) employees 테이블에 regId, deptId, jobId 컬럼 추가. 
자료형은 regions, departments, jobs 테이블의 컬럼 구조와 동일하게 지정.
*/
ALTER TABLE employees 
    ADD (regId VARCHAR2(6), deptId VARCHAR2(7), jobId VARCHAR2(6));
...
UPDATE jobs
    SET min_basicPay = DECODE(jobId, 'job001', 1000000, 'job002', 1500000,
                                    'job003', 2000000, 'job004', 2500000);
...
SELECT e.empId, e.name_, e.ssn, e.hiredate, e.phone, r.reg_name, d.dept_name,
        j.job_title, e.basicpay, e.extrapay, (e.basicpay + e.extrapay) pay
