...
function userIdCheck() {
	...
	if(!/^[a-z][a-z0-9_]{4,9}$/i.test(userId)) { 
		...
		$("#userId").focus();
		$("#userId").parents().next(".help-block").html(str);
		return;
	}
	...
	$.ajax({
		...
		,success:function(data){
			...
			if(passed=="true"){
				var str="<span style='color:blue; font-weight:bold;'>"+userId+"</span> 아이디는 사용 가능 합니다.";
				$("#userId").parents().next(".help-block").html(str);
			} else {
				var str="<span style='color:red; font-weight:bold;'>"+userId+"</span> 아이디는 사용할 수 없습니다.";
				$("#userId").parents().next(".help-block").html(str);
				$("#userId").val("");
				$("#userId").focus();
			}
